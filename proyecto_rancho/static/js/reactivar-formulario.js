...
$('#btnActivarCrearCuenta').on('click', async function() {
    const url = $(this).data('url');
    csrfToken = document.querySelector('[name=csrfmiddlewaretoken]').value;
    const response = await fetch(url, {
        method: "POST",
    ...
    if (data.success) {
        ...
        $(this).data('activado', true);
        $(this).addClass('btn-secondary').removeClass('btn-warning');
        $(this).text("La creación de cuentas está activa hasta las " + data.expira);
    } else {
        alert("❌ " + (data.message || "No se pudo activar la creación de cuentas."));
    }
});